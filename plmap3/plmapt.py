#!/usr/bin/python3
"""
This is a generic module for parallelization ( using Threads ) in python3
...
def plmapt(func, args=[], kwargs=[], threads=10, default_output=None, sort_output=True, progress_bar=False):
    ...
    input_queue = Queue()
    output_errors = [[] for _ in range(big_len)]
    output_values = [[] for _ in range(big_len)]
    ...
    list_of_workers = []
    for i in range(min(threads, big_len)):
